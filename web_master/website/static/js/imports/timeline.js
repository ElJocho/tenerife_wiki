// code by and credits to Sol Vitkin: https://github.com/svitkin/leaflet-timeline-slider
  ...
  setupStartStyles: function () {
    style = `
            .control_container { 
                ...
                padding: ${that.options.topBgPadding} ${
      that.options.rightBgPadding
    } ${that.options.bottomBgPadding} ${that.options.leftBgPadding};
            }
            .range {
                position: relative;
    ...
    for (var i = 0; i < prefs.length; i++) {
      ...
      style +=
        ...
        `{background: linear-gradient(to right, ${
          that.coverBackgroundRGBA
        } 0%, ${that.coverBackgroundRGBA} ${coverVal}%, ${
          that.options.activeColor
        } 0%, ${that.options.activeColor} ${val}%, ${
          that.options.inactiveColor
        } ${val}%, ${that.options.inactiveColor} ${100 - coverVal}%, ${
          that.coverBackgroundRGBA
        } ${100 - coverVal}%, ${that.coverBackgroundRGBA} 100%)}`;
    }
